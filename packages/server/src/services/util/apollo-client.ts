...
type Requester<C = Record<string, unknown>> = <R, V>(
    doc: DocumentNode,
    vars?: V,
    options?: C,
) => Promise<R>;
...
export function getUniswapV3Sdk<C>(
    client: ApolloClient<C>,
): ReturnType<typeof UniswapV3Types.getSdk> {
    const requester = generateRequester(client);
    return UniswapV3Types.getSdk(requester);
}

export function getBitquerySdk<C>(
    client: ApolloClient<C>,
): ReturnType<typeof BitqueryTypes.getSdk> {
    const requester = generateRequester(client);
