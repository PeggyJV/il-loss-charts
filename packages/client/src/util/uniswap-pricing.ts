...
export function calculatePoolEntryData(
    ...
    if (entryToken === 'ETH') {
        ...
        if (
            pairData.token0.symbol !== 'WETH' &&
            pairData.token1.symbol !== 'WETH'
        ) {
            ...
            const currentPriceRatio = new BigNumber(pairData.reserve0).div(
                pairData.reserve1,
            );

            // Deduct fee from amount we can swap
            const purchasingPower = new BigNumber(0.997).times(
                expectedPoolToken0,
            );
            ...
            expectedPoolToken1 = updatedReserve1
            ...
            expectedPriceImpact = new BigNumber(newPriceRatio)
                .minus(currentPriceRatio)
                .div(currentPriceRatio)
                .times(100)
                .abs()
                .toFixed(2);
        } else {
            ...
            expectedPoolToken0 = updatedReserve0
        ...
        console.warn(
            `Entry token ${entryToken} does not belong to pair - could not calculate price impact`,
        );
    }
